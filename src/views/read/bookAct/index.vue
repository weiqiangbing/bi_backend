<template>
  <div class="app-container book-act-wrap">	 
    <!-- 筛选 -->
    <filter-inputs ref="filterDom" @handleFilter="handleFilter" @handleDownload="handleDownload" @handleShowRow="handleShowRow"
    @handleShowEcharts="handleShowEcharts" :tableData="tableData" :checkedList="checkedList" :downloadLoading="downloadLoading" :filterConfig="pageConfig.bookAct"
...
<script>
import { getList } from '@/api/read/book_act'
...
import { setBookActList, getBookActList, removeBookActList} from '@/utils/auth'
...
export default {
  ...
  components: {
	...
	data() {
      ...
      noRepeatSum: JSON.parse(JSON.stringify(pageTableConfig.bookAct.noRepeatSum)),
      sortSum: JSON.parse(JSON.stringify(pageTableConfig.bookAct.sortSum)),
		}
  },
  filters: {
    outSiteText,
	...
	created() {
    let pageCount = localStorage.getItem('book_act_pagecount')
    if(pageCount){
  ...
  mounted() {
    ...
    computeThs(num){
      ...
      if(getBookActList().length > 0){
        this.checkedList = getBookActList()
      }else{
        this.checkedList = pageSelectTh.bookactTh
      }  
    }, 
    listData(isEch) {		
    ...
    checkListChange(){
      setBookActList(this.checkedList)
    },   
    ...
    handleFilter(listQuery){
      listQuery.order = 'bookshelf desc'
      this.currentPage = 1
    ...
    handleSizeChange(val){
      localStorage.setItem('book_act_pagecount', val)
      this.currentPage = 1
      this.pageCount = val
			this.loading = true
			this.listData()
