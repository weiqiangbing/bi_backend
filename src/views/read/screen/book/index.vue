...
<el-row style="height: 100%;">
	...
	<el-col class="border-left" :span="8">
		<div class="step-tit"><el-tag>第二步</el-tag></div class="step-tit">
		<div>			
			<!-- <p v-show="postData.content.searches.length && postData.content.site_code == 0 || postData.content.site_code"><el-tag>站点： {{siteList1[postData.content.site_code]}}</el-tag></p> -->
			<el-steps :active="postData.content.searches.length" direction="vertical" align-center>	
				<el-step v-for="(item, index) in postData.content.searches" :title="handlesData[item.db_table]" :description="desMsg(item, index)"></el-step>
			</el-steps>
		</div> 
		<div>
			<el-form>
	...
	<el-col class="border-left" :span="8">
		<div class="step-tit"><el-tag>第三步</el-tag></div class="step-tit">
		<div class="result-data">
				...
				if(this.dialogForm[1]=='time'){
					if(this.isRelative){
						...
						if(this.dateObjs.end_time){
							...
							this.dialogForm[1] = 'time'
							this.dialogForm[2] = time1+','+time2
						}
					}else{
						// this.dialogForm[0] = this.dialogForm[0]
						this.dialogForm[1] = '~'
