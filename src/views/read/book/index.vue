...
<script>
import { getList } from '@/api/read/user'
...
import { setBookList, getBookList, removeBookList} from '@/utils/auth'
...
export default {
  ...
  components: {
	...
	data() {
      ...
      noRepeatSum: JSON.parse(JSON.stringify(pageTableConfig.bookData.noRepeatSum)),
      sortSum: JSON.parse(JSON.stringify(pageTableConfig.bookData.sortSum)),
		}
  },
  filters: {
    outSiteText,
    ...
    // console.log('pageTableConfig.bookData',pageConfig);
  ...
  computed:{
    ...
    computeThs(num){
      ...
      // console.log(JSON.stringify(this.defaultCheckedList));
      if(getBookList().length > 0){
        this.checkedList = getBookList()
      }else{
        this.checkedList = pageSelectTh.booksTh
      }  
    }, 
    listData(isEch) {	
