...
<script>
import { getList } from '@/api/read/user_pay'
...
import { setPayUserList, getPayUserList, removePayUserList} from '@/utils/auth'
...
export default {
  ...
  components: {
	...
	data() {
      ...
      checkedSum: JSON.parse(JSON.stringify(pageTableConfig.payUser.checkedSum)),
      sortSum: JSON.parse(JSON.stringify(pageTableConfig.payUser.sortSum)),
		}
	},
	created() {
    let pageCount = localStorage.getItem('user_pay_pagecount')
    if(pageCount){
  ...
  mounted() {
    ...
    computeThs(num){
      ...
      if(getPayUserList().length > 0){
        this.checkedList = getPayUserList()
      }else{
        this.checkedList = pageSelectTh.payuserTh
      } 
    },      
    listData(isEch) {		
    ...
    checkListChange(){
      setPayUserList(this.checkedList)  
    },  
    ...
    handleSizeChange(val){
      localStorage.setItem('user_pay_pagecount', val)
      this.currentPage = 1
      this.pageCount = val
			this.loading = true
			this.listData()
    ...
    handleRenderHeader(h, { column, $index },item){
      return h(
        'div',{
          style: 'width: 100px;padding: 0;margin: 0;text-align: left;white-space:normal; word-break:break-all;line-height: 16px;'
        },
        [ 
          h('span', column.label),
          h('el-popover', 
            ...
            [
              h('span', { slot: 'reference',class:'font-normal'},[
                  h('i', {class:'el-icon-question',style: 'color:#409eff;margin-left:5px;font-size:15px;margin-top: 5px;'}),
              ])
            ])
        ])
    },
