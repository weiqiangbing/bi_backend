...
<script>
import { getList } from '@/api/read/user_buy'
...
import { setCostUserList, getCostUserList, removeCostUserList} from '@/utils/auth'
...
export default {
  ...
  components: {
	...
	data() {
      ...
      noRepeatSum: JSON.parse(JSON.stringify(pageTableConfig.costUser.noRepeatSum)),
      sortSum: JSON.parse(JSON.stringify(pageTableConfig.costUser.sortSum)),
		}
  },
  filters: {
    outSiteText,
	...
	created() {
    let pageCount = localStorage.getItem('user_cost_pagecount')
    if(pageCount){
  ...
  mounted() {
    ...
    computeThs(num){
      ...
      if(getCostUserList().length > 0){
        this.checkedList = getCostUserList()
      }else{
        this.checkedList = pageSelectTh.costuserTh
      } 
    },      
    listData(isEch) {		
    ...
    checkListChange(){
      setCostUserList(this.checkedList)
    },   
    handleFilter(listQuery){
      listQuery.order = 'chapters_total desc'
      this.currentPage = 1
    ...
    handleSizeChange(val){
      localStorage.setItem('user_cost_pagecount', val)
      this.currentPage = 1
      this.pageCount = val
			this.loading = true
			this.listData()
