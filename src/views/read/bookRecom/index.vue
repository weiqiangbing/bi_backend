<template>
  <div class="app-container user-recom-wrap">	 
    <!-- 筛选 -->
    <filter-inputs ref="filterDom" @handleFilter="handleFilter" @handleDownload="handleDownload" @handleShowRow="handleShowRow"
    @handleShowEcharts="handleShowEcharts" :tableData="tableData" :checkedList="checkedList" :recomeData="recomeData" :downloadLoading="downloadLoading" :filterConfig="pageConfig.bookRecom"
...
<script>
import { getList } from '@/api/read/user_recom'
...
import { setBookRecomList, getBookRecomList, removeBookRecomList} from '@/utils/auth'
...
export default {
  ...
  components: {
	...
	data() {
      ...
      noRepeatSum: JSON.parse(JSON.stringify(pageTableConfig.bookRecom.noRepeatSum)),
      sortSum: JSON.parse(JSON.stringify(pageTableConfig.bookRecom.sortSum)),
		}
  },
  filters: {
    outSiteText,
	...
	created() {
    let pageCount = localStorage.getItem('book_recom_pagecount')
    if(pageCount){
  ...
  mounted() {
    ...
    computeThs(num){
      ...
      if(getBookRecomList().length > 0){
        this.checkedList = getBookRecomList()
      }else{
        this.checkedList = pageSelectTh.bookrecomTh
      }  
    },   
    listData(isEch) {		
      ...
      let params = {
        ...
        tj_page: this.listQuery.tj_page,
      ...
      getList(newParams).then(res => { 
        ...
        let recomeData = res.data.tj_page_list
        let rearr = []
        for(let i in recomeData){
          rearr.push({value: i, label: recomeData[i]})
        }
        this.recomeData = rearr
        if(!isEch){
            this.tableData = [...res.data.collection]
    ...
    checkListChange(){
      setBookRecomList(this.checkedList)
    },    
    handleFilter(listQuery){
      listQuery.order = 'uv desc'
      this.currentPage = 1
    ...
    handleSizeChange(val){
      localStorage.setItem('book_recom_pagecount', val)
      this.currentPage = 1
      this.pageCount = val
			this.loading = true
			this.listData()
