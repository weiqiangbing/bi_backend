  ...
  <div class="app-container user-wrap add_data">	 
    ...
    <div class="table_nopad" v-loading="loading">    
      <el-table
        ...
        style="width: 100%">
          <!-- 正常表单列 -->
          <el-table-column prop="reg_date" label="时间" v-if="filterAuth('reg_date')" :fixed="true" width="96" align="center"></el-table-column>
          <el-table-column prop="site_id" label="站点" v-if="show_field.indexOf('site_id') != -1 && filterAuth('site_id')" :fixed="true" width="80" :formatter="formatTdValue" align="center"></el-table-column>
          <el-table-column prop="platform" label="平台" v-if="show_field.indexOf('platform') != -1 && filterAuth('platform')" :fixed="true" width="70" :formatter="formatTdValue" align="center"></el-table-column>
          <el-table-column prop="reg_user" label="新增用户" width="100" align="center" v-if="filterAuth('reg_user')"></el-table-column>
          <el-table-column prop="total_order_money" label="累计充值" width="100" align="center" v-if="filterAuth('total_order_money')"></el-table-column>
          <el-table-column prop="daa_cost" label="成本" width="100" align="center" v-if="filterAuth('daa_cost')"></el-table-column>
          <el-table-column prop="recovery_day" label="回本天数 / 预计回本天数" width="110" align="center" v-if="filterAuth('recovery_day')"></el-table-column>
          <el-table-column prop="last_ratio" label="累计回本率" width="110" align="center" v-if="filterAuth('last_ratio')"></el-table-column>

          <el-table-column v-for="(val, index) in monthDays" :prop="'field_'+val" :key="index" v-if="filterAuth('field_'+val)" :label="'第'+val+'天充值'" width="100"  align="center">
          ...
          <el-table-column v-for="index in [30,60,90,120,150]" :prop="'field_show_'+index" v-if="filterAuth('field_show_'+index)" :label="'第'+index+'-'+(index+30)+'天'" :key="index" width="100" align="center">
            <template slot-scope="props">
...
export default {
  ...
  created() {
    let pageCount = localStorage.getItem('extend_new_add_data_pagecount')
    if(pageCount){
  ...
  mounted() {
    ...
    handleSizeChange(val){
      localStorage.setItem('extend_new_add_data_pagecount', val)
      this.currentPage = 1
      this.pageCount = val
			this.loading = true
			this.listData()
