<template>
  <div class="app-container user-wrap">	 
    ...
    @handleShowRow="handleShowRow" @handleShowEcharts="handleShowEcharts" @handleSaveEcharts="handleSaveEcharts" :tableData="tableData" :checkedList="checkedList" :downloadLoading="downloadLoading" :filterConfig="pageConfig.uvRecords" :groupListData="groupListData"></filter-inputs>
    <!-- 显示隐藏列 -->
    ...
    <div class="main-table">    
      <el-table
        ...
        <template v-for="(item) in tableColumn">
          ...
          <!-- <el-table-column
            v-bind:key="item.prop"
            v-if="noRepeatSum[item.prop] && checkedSum[item.prop] && item.prop != 'site_id' && item.prop != 'platform' && item.prop != 'event_type'"
            :prop="item.prop"
            :label="item.label"
            :show-overflow-tooltip="true"
            :fixed="item.fixed ? item.fixed : false"
          ...
          <el-table-column width='100' align="center" label="推广" :prop="item.prop" v-if="item.prop === 'event_type' && checkedSum[item.prop]" v-bind:key="item.label">
            <template slot-scope="props">            
              <span >{{props.row.event_type==1000?'推广':'未推广'}}</span>
            </template>
          </el-table-column> -->
        </template>
      </el-table>
...
<script>
import { getList } from '@/api/record/uvRanking'
...
import { setuvRankingList, getuvRankingList, removeuvRankingList} from '@/utils/auth'
...
export default {
  ...
  components: {
	...
	data() {
      ...
      noRepeatSum: JSON.parse(JSON.stringify(pageTableConfig.uvRecords.noRepeatSum)),
      checkedSum: JSON.parse(JSON.stringify(pageTableConfig.uvRecords.checkedSum)),
      // checkedSum: Object.assign({}, pageTableConfig.uvRecords.checkedSum),
      sortSum: JSON.parse(JSON.stringify(pageTableConfig.uvRecords.sortSum)),
		}
  },
  filters: {
    outSiteText,
...
created() {
    ...
    // console.log('pageTableConfig.uvRecords',pageTableConfig.uvRecords.checkedSum);
    ...
    //   if(pageTableConfig.uvRecords.checkedSum[this.tableColumn[i].prop]){
    ...
    //     if(getuvRankingList().length > 0){
    //       this.checkedList = getuvRankingList()
    //       this.isChecked()   
    //     }else{
  ...
  mounted() {
    ...
    checkListChange(){
      setuvRankingList(this.checkedList)
      this.isChecked()   
    },  
